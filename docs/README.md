...
- **要看走过的路** → [2026-09-13-完整日志.md](2026-09-13-完整日志.md)（**完整工作日志**：2026-09-11 → 09-13 的全部提交、问题根因、修法与实测证据）
...
| 文档 | 内容 |
|---|---|
| [00-项目说明.md](00-项目说明.md) | **完整项目说明（权威）**：一页速览、背景与硬约束、加载原理、功能全表、18 个模块参考、设计 token、33 个套件的验证体系、上游缺陷反制清单、维护手册、被推翻的结论 |
...
| 文档 | 记录了什么 |
|---|---|
| [2026-09-13-完整日志.md](2026-09-13-完整日志.md) | **完整工作日志（推荐先读这一篇）**：2026-09-11 立项 → 09-12 覆盖式抽屉/审查/批次修复 → 09-13 真机三连修 + 完整项目说明；每个问题都带根因、处理、验证与提交号 |
| [2026-09-12-ui-audit.md](2026-09-12-ui-audit.md) | 全面 UI 审查；§3 的清单大多已由同文 §5b–5h 落地，行号已过期。仍开放：工具卡取舍、`layoutRef` 在纯高度变化下的复位、抽屉设置入口失败时无反馈 |
| [2026-09-11-session-log.md](2026-09-11-session-log.md) | 早期一轮逐场调研日志（已被完整日志收录） |
...
`docs/` 下除了文档还有一个可复用脚本，之前没进任何索引：

| 脚本 | 用途 |
|---|---|
| [tools/slot-diff.ps1](tools/slot-diff.ps1) | 对两颗 DSH 安装树机械提取 slot 面并逐个对照签名（用法与实测结果见 [01-final-plan.md](01-final-plan.md) 的 slot 对照一节） |
| [/STATUS.md](../STATUS.md) | 2026-09-11 的项目状态快照。**已过期**，只作历史与踩坑记录（§4 的结论仍成立），当前状态看 [00-项目说明.md](00-项目说明.md) |
