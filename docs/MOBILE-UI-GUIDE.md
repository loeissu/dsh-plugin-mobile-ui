# dsh-plugin-mobile-ui · 使用与维护指南

> 想了解**整个项目**（背景、约束、全部模块、设计 token、上游缺陷反制、维护手册）请看
> [00-项目说明.md](00-项目说明.md)；本文聚焦日常使用与维护。
...
- 主题三列横排（浅色 / 深色 / 跟随系统）  
- 弹层打开时隐藏顶栏「导航」，避免压标题（规则在 `PHONE_MEDIA` 块里：横屏同样要隐藏，因为横屏下抽屉根节点 z-index 高于承载弹层的侧栏列）
- **标题行给足高度（`_navTitle` min-height 44px）**：标题行原本只有 22px 高，却放着 32px 的关闭按钮与 28px 的「打开配置文件」——桌面下 nav 是左栏、戳出去没东西可撞，**手机上 tether 把 nav 变成标题下方的横排 tab 条，两个按钮就直接压在那条上**（实测重叠 18px，× 盖住被选中的「移动端」胶囊）。不是把控件改小，而是把行高还给它，关闭按钮的命中层实测 45px
- **五个 tab 必须放得下（不横滚）**：授权间距下五格共需 **453px** → 横滚，而选中的是最后一格（本插件的「移动端」）时条带会滚到右端，**最左的「通用设置」被裁成「设置」**。只回收间距、不动字号与内容：格子 padding 11→6px（−50）、图标与文字间距 8→4px（−20）、条带 gap 4→2px（−8）→ **实测 377px 装进 392px（412 屏），留约 15px 余量**（7px padding 时是刚好 100% 占满，字体度量一变就会再裁）
- **≤408px 时隐藏 tab 图标**：更窄的屏上五格文字怎么压都放不下（实测 360px 屏只有 340px 可用、320px 屏只有 300px，而内容要 377px），所以隐藏图标——每格省 20px，标签保留 → **实测 285px，320px 屏也放得下**。`verify-settings-chrome.mjs` 在 320/360/412/430 四个宽度上都断言
...
src/
  ...
  client/
    ...
    Drawer.tsx                  已废弃侧栏接管（默认关，勿开）
    viewport.ts                 键盘适配
    tether-compat.ts            tether 反制
    settings-chrome.ts          主机设置弹层排版 + 触控层
    settings-swipe.ts           设置页左右滑动换分区
    conversation-chrome.ts      宿主会话控件命中区（横竖屏）+ 标题让位/指标行（仅窄屏）
    connection-recovery.ts      回前台自动重连
    keyboard-debug.ts           诊断徽章（默认关）
lib/                            构建产物（不入 git）
tools/                          CDP 验收脚本（verify-*.mjs）与探针
tools/retired/                  断言已放弃设计的脚本（见其 README）
docs/                           报告与日志（索引见 docs/README.md）
```
